...
abstract class _CreateControllerBase with Store {
  ...
  _CreateControllerBase(this.boardController) {
    _init();
  }
  ...
  Future createBoard(BoardStore boardStore) async {
    ...
    boardController.boards.insert(0, newBoard);
  ...
  Future updateBoard(BoardStore boardStore) async {
    ...
    for (var i = 0; i < boardController.boards.length; i++) {
      if (boardController.boards[i].id == newBoard.id) {
        boardController.boards[i] = newBoard;
      }
    }
